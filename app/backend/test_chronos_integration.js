...
async function runTests() {
    ...
    let initialHealth = await chronosClient.checkChronosHealth();
    let pyProcess = null;
    ...
    if (!initialHealth.reachable) {
        ...
        console.log("  Python Chronos service is already running and healthy on port 5001.");
    }
    ...
    // TEST 4: Controlled Fallback Verification
    console.log("TEST 4: Verifying controlled baseline fallback when Chronos is unreachable...");
    if (pyProcess) {
        pyProcess.kill();
        await sleep(1000);
    }

    // Force an offline call to verify fallback behavior
    const origUrl = process.env.CHRONOS_SERVICE_URL;
    process.env.CHRONOS_SERVICE_URL = "http://127.0.0.1:59999";
    ...
    if (origUrl) process.env.CHRONOS_SERVICE_URL = origUrl; else delete process.env.CHRONOS_SERVICE_URL;

    console.log("  Controlled fallback result:", postKillForecast);
    assert.strictEqual(postKillForecast.ok, false, "Should return ok: false when service is unreachable");
    assert.strictEqual(typeof postKillForecast.fallbackReason, 'string', "fallbackReason should be a string");
